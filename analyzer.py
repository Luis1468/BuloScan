...
# Estas listas son el "conocimiento" del analizador.
# Cuantas más palabras y patrones, más preciso será el resultado.
# =============================================================================
...
# Si el texto contiene alguna de estas combinaciones → penalización muy alta
...
class Analyzer:
    """
    Analizador de credibilidad de noticias de BuloScan.

    Combina tres técnicas complementarias para calcular un score de 0 a 1
    que indica la probabilidad de que una noticia sea falsa:

        1. Análisis de fuente   → ¿De dónde viene la noticia?
        2. Análisis lingüístico → ¿Cómo está escrita?
        3. Análisis de sentimiento (TextBlob) → ¿Es extremadamente emotiva?

    El resultado final es un score_fake entre 0.0 (muy fiable) y 1.0 (muy falsa),
    junto con un veredicto legible: FIABLE, SOSPECHOSA o FALSA.
    """
    ...
    def analizar(self, noticia):
        """
        Analiza una noticia y devuelve el resultado completo.

        Args:
            noticia (dict): Noticia normalizada desde NewsSensor. Debe tener:
                titulo, descripcion, url, dominio, fuente, es_fiable.

        Returns:
            dict: Noticia original enriquecida con:
                - score_fake (float): Probabilidad de ser falsa (0.0 - 1.0).
                - veredicto (str): "FIABLE", "SOSPECHOSA" o "FALSA".
                - detalle (dict): Desglose de cada factor analizado.
        """
        ...
        # --- Score final: media ponderada de los tres factores ---
        # La fuente tiene más peso (40%) porque es el indicador más fiable.
    ...
    def _analizar_fuente(self, dominio, es_fiable):
        """
        Evalúa la credibilidad de la fuente de la noticia.

        Lógica:
        - Si el dominio está en nuestra lista de fuentes fiables → score bajo (bueno)
        - Si el dominio está en nuestra lista de sospechosos → score alto (malo)
        - Si es desconocido → score neutro (0.5)

        Args:
            dominio (str): Dominio extraído de la URL (ej: "elpais.com").
            es_fiable (bool): Flag calculado por NewsSensor.

        Returns:
            tuple(float, dict): Score de fuente y detalle explicativo.
        """
        if es_fiable:
    ...
    def _analizar_linguistico(self, titulo, texto):
        """
        Analiza el lenguaje del título y descripción buscando patrones
        asociados a noticias falsas o sensacionalistas.

        Args:
            titulo (str): Titular original de la noticia.
            texto (str): Título + descripción en minúsculas.

        Returns:
            tuple(float, dict): Score lingüístico y detalle explicativo.
        """
        penalizacion  = 0.0
        bonificacion  = 0.0
        hallazgos     = []
    ...
    def _analizar_sentimiento(self, texto):
        """
        Usa TextBlob para medir la subjetividad y polaridad emocional del texto.

        Las noticias falsas tienden a ser muy emotivas y subjetivas.
        Una noticia objetiva y neutral tiene baja subjetividad.

        TextBlob devuelve:
        - polarity:    -1.0 (muy negativo) a +1.0 (muy positivo)
        - subjectivity: 0.0 (objetivo) a 1.0 (muy subjetivo)

        Args:
            texto (str): Texto a analizar (título + descripción).

        Returns:
            tuple(float, dict): Score de sentimiento y detalle explicativo.
        """
        try:
            blob = TextBlob(texto)
    ...
    def _interpretar_sentimiento(self, subjetividad, polaridad):
        """Convierte los valores numéricos de TextBlob en texto legible."""
        if subjetividad > 0.7:
...
# Permite usar el analizador sin instanciar la clase manualmente
# =============================================================================
...
def analizar_noticia(noticia):
    """
    Función de acceso rápido al analizador.
    Se importa directamente desde main.py y el resto del backend.

    Args:
        noticia (dict): Noticia normalizada desde NewsSensor.

    Returns:
        dict: Noticia con score_fake, veredicto y detalle añadidos.
    """
